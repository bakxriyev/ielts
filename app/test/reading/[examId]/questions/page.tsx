...
export default function ListeningTestPage({ params }: { params: Promise<{ examId: string }> }) {
  ...
  const handleAnswerChange = (questionId: string, answer: any, questionIdentifier?: string) => {
    ...
    if (!isEmptyAnswer) {
      ...
      if (actualQuestionType === "TFNG" && questionIdentifier && questionIdentifier.includes("_map_")) {
        ...
        const parts = questionIdStr.split("_table_")
        const baseQuestionId = parts[0]
        ...
        answersArray.push({
          ...
          examId: Number.parseInt(examId),
          question_type: actualQuestionType,
          answer: answer, // Save just the answer string, not an object
          l_questionsID: uniqueLQuestionsID,
        })

        const cellKey = `${baseQuestionId}_table_${rowIndex}_${cellIndex}`
        setAnswers((prev) => ({
          ...prev,
          [cellKey]: answer,
        }))
        ...
        for (const [key, value] of Object.entries(newSummaryAnswers)) {
  ...
  const isSubQuestionAnswered = (question: any, subIndex: number): boolean => {
    ...
    if (question.q_type === "TABLE_COMPLETION") {
            ...
            for (let c = 0; c < row.cells.length; c++) {
              ...
              if (isEmptyOrUnderscore) {
                if (currentCellIndex === subIndex) {
                  const cellKey = `${questionIdStr}_table_${r}_${c}`
                  return answers[cellKey] !== undefined && answers[cellKey] !== ""
                }
                currentCellIndex++
              }
            }
      ...
      if (summaryAnswers && typeof summaryAnswers === "object") {
        const answerValue = summaryAnswers[Object.keys(summaryAnswers)[subIndex]]
  ...
  const clearAnswer = (questionIdentifier: string, questionType: string) => {
    ...
    if (questionType === "TABLE_COMPLETION") {
      const baseQuestionId = questionIdentifier
      const question = getAllQuestions().find((q) => q.id.toString() === baseQuestionId)

      if (question && question.rows) {
        for (let r = 0; r < question.rows.length; r++) {
          const row = question.rows[r]
          if (row.cells) {
            for (let c = 0; c < row.cells.length; c++) {
              ...
              const isEmptyOrUnderscore = cell === "" || cell === "_" || (typeof cell === "string" && /_+/.test(cell))
              if (isEmptyOrUnderscore) {
                const cellKey = `${baseQuestionId}_table_${r}_${c}`
                delete answersObject[cellKey]
                                    ...
                                    >
                                      <td className="p-3 font-bold text-gray-900 border-r-2 border-black">
                                        <div className="flex items-center gap-2">
                                          <span
                                            className="bg-white border-2 border-[#4B61D1] w-6 h-6 rounded flex items-center justify-center text-sm font-bold"
                                            style={{ fontSize: `${textSize * 0.875}px` }}
                                          >
                                            {actualQuestionNum}
                                          </span>
                            ...
                            {question.rows?.map((row: any, rowIndex: number) => (
                              <tr key={rowIndex}>
                                {row.cells?.map((cell: string, cellIndex: number) => {
                                  ...
                                  if (isEmptyOrUnderscore || hasUnderscores) {
                                    const cellKey = `${questionId}_table_${rowIndex}_${cellIndex}`
                                                ...
                                                if (/_+/.test(part)) {
                                                  return (
                                                    <Input
                                                      ...
                                                      onChange={(e) =>
                                                        handleAnswerChange(
                                                          cellKey, // Pass individual cell key
                                                          e.target.value,
                                                        )
                                                      }
                                  ...
                                  question.options
                                    ...
                                    .map((optionKey: string) => (
                                      <div
                                        ...
                                        className="bg-white border border-gray-400 rounded-md px-3 py-1 cursor-move hover:border-[#4B61D1] hover:shadow transition-all"
                                      >
